...
import { getClinicServices, getMyCompanies } from "@/services/clinic";
...
const initialFormValues: ClinicFormValuesType = {
  company: {
    id: "",
    name: ""
  },
...
const ClinicForm = () => {
  ...
  useEffect(() => {
    (async () => {
      const { response, error } = await getEmployees(formValues?.company?.id);
  ...
  const handleFormChange = (
    ...
    key?: string,
    parentKey?: "company" | "services" | "employees",
    index?: number
  ) => {
    if (parentKey === "company") {
      const data = companies?.filter((item) => item.value === value)?.[0];

      const values: ClinicFormValuesType = {
        ...initialFormValues,
        [parentKey]: {
          id: data.value,
          name: data.label
  ...
  const onFormSubmit = async (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    formValues?.employees?.forEach((employee) => {
      employee?.service?.forEach((service) =>
        payload.push({
          date: employee?.date as Date,
          employee: employee?.id,
          company: formValues?.company?.id,
          clinic: currentUser?.clinic?._id,
      ...
      <div className="w-full flex flex-col justify-start items-start bg-white gap-[12px]">
        ...
        <div className="flex justify-start items-center w-full p-[10px] flex-wrap gap-[30px]">
          <Select
            ...
            showSearch={true}
            className="basis-[30%]"
            onChange={(val) => handleFormChange(val, "id", "company")}
